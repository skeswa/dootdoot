...
- **Localized dash breath.** A dash tagged its _entire_ preceding segment as a breathy
...
- **Integrated breath.** The aspiration noise was _stationary_ value-noise (a buzzy ~6.3 kHz
...
### 9.4 Decision: release packaging is downstream of the same binary

The friend-friendly macOS install path is a custom Homebrew tap backed by prebuilt
GitHub Release artifacts, generated by `dist` (`cargo-dist`). Each tagged release builds
only the `dootdoot` binary crate for Apple Silicon and Intel Macs, publishes a Homebrew
formula that fetches those archives, and also uploads a shell installer plus updater for
users who do not use Homebrew.
The macOS binary jobs are routed to the dedicated `dootdoot-macos-arm64` self-hosted
Apple Silicon runner; `dist` cross-builds the Intel archive from that same host.

This packaging layer is deliberately downstream of the normal release build:

- it uses the same binary that embeds `assets/dootdoot_asset_v1.doot`;
- it never runs `xtask` or downloads model inputs;
- it excludes the workspace's build-time `xtask` crate from release artifacts; and
- it is guarded by a static release smoke check plus `dist plan` before tags are pushed.
...
- **Additional packaging channels** beyond the initial Homebrew/prebuilt macOS release
  path — crates.io, package-manager submissions outside the custom tap, Windows installers,
  and notarized/codesigned macOS artifacts.
